...
const config: Config = {
  // Dark mode is toggled via [data-theme='dark'] attribute (driven by next-themes)
  ...
  theme: {
    // Custom breakpoints matching isomorphic (adds xs and 3xl/4xl)
    screens: {
      xs: '480px',
      sm: '640px',
      md: '768px',
    ...
    extend: {
      // ─── Colors ──────────────────────────────────────────────────────────
      // All colors point to CSS variables so light/dark switching is automatic.
      colors: {
        gray: {
      ...
      boxShadow: {
        ...
        roundedCard:  '0px 4px 6px -4px rgba(0,0,0,0.1), 0px 10px 15px -3px rgba(0,0,0,0.1)',
      },
